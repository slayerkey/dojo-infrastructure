...
export async function postDailyDigest(env, stub, channelId, options = {}) {
  if (!channelId) throw new Error("No Daily Digest channel is configured.");
  const model = await buildDailyDigestModel(env, stub);
  const payload = buildDailyDigestPayload(model, {
    title: options.manual ? "📊 Daily Digest — Manual Check" : "📊 Daily Digest",
...
export async function buildDailyDigestModel(env, stub) {
...
export function buildDailyDigestPayload(model, options = {}) {
  ...
  return {
    content: "",
    embeds: [{
      title: options.title || "📊 Daily Digest",
      description: [
        "**Who shows up here?**",
