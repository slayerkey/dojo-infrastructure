...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/health") {
      ...
      try {
        ...
        body.discord.rr_presentation = "v10";
        body.discord.verify_all_mode = "durable-object-batched-progress";
  ...
  async scheduled(controller, env, ctx) {
    // Never call the old membership reconciler. It was the destructive path.
    // The cron only keeps the Discord Gateway alive.
    if (env.DISCORD_GATEWAY) {
      ctx.waitUntil(
...
export class DiscordGateway extends DiscordGatewayV9 {
  ...
  async alarm() {
    ...
    // Keep each alarm well below the Free plan external subrequest limit.
...
async function getRankIconUrl(rankName) {
  ...
  const cacheKey = "__dojoValorantRankIconsV10";
  const timeKey = "__dojoValorantRankIconsV10At";
...
async function takeCooldown(command, discordUserId, env) {
  ...
  const key = `cooldown:v10:${command}:${discordUserId}`;
  const now = Date.now();
