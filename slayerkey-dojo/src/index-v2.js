...
const COMMANDS_VERSION = "2026-08-21-v2";
const WHOP_CONNECTED_ACCOUNTS_URL = "https://whop.com/@me/settings/connected-accounts/";
const EPHEMERAL = 64;
const MANUAL_VERIFY_TTL_SECONDS = 7 * 24 * 60 * 60;
const COMMAND_TIMEOUT_MS = 15000;
const encoder = new TextEncoder();

const ACCESS_STATUSES = new Set(["active", "trialing", "canceling"]);
const RR_COMMANDS = new Set(["linkriot", "sync", "rr", "rrleaderboard"]);
const OWNER_COMMANDS = new Set(["verify-all", "manualverify", "manualunverify"]);
...
async function handleDiscordInteraction(request, env, ctx) {
  ...
  if (!discordUserId) {
    return interactionMessage("I could not determine your Discord user ID.", true);
  }

  if (OWNER_COMMANDS.has(commandName) && discordUserId !== String(env.DISCORD_OWNER_USER_ID || "")) {
    return interactionMessage("This command is owner only.", true);
  }

  if (RR_COMMANDS.has(commandName) && !hasDojoRole) {
    return interactionMessage(
      "You need the Training Dojo role to use this command. Run **/verify** first.",
      true,
    );
  }
...
async function runDeferredCommand(interaction, env) {
  ...
  try {
    const message = await withTimeout(
      ...
      COMMAND_TIMEOUT_MS,
      "That command took too long to finish. Please try it again.",
    );
    await editOriginalInteraction(interaction, env, message);
  } catch (error) {
...
function withTimeout(promise, timeoutMs, timeoutMessage) {
  return Promise.race([
    promise,
    new Promise((resolve) => {
      setTimeout(() => resolve(timeoutMessage), timeoutMs);
    }),
  ]);
}
...
async function executeDiscordCommand(interaction, env) {
  ...
  if (commandName === "manualverify") {
    return commandManualVerify(interaction, discordUserId, env);
  }

  if (commandName === "manualunverify") {
    return commandManualUnverify(interaction, discordUserId, env);
  }
...
async function commandVerify(discordUserId, env) {
  if (discordUserId === String(env.DISCORD_OWNER_USER_ID || "")) {
    ...
    if (!roleResult.success) {
      return "I found your owner account, but I could not add the Training Dojo role. Check the bot role permissions and role order.";
    }
    ...
    if (env.RR_TRACKER) {
      try {
        ...
        console.warn("Could not mark owner active in RR tracker:", error);
      }
    }
  ...
  return (
    "I could not verify an active Training Dojo membership for this Discord account yet.\n\n" +
    `Make sure **this same Discord account** is connected to Whop: [Connect Discord to Whop](${WHOP_CONNECTED_ACCOUNTS_URL})\n\n` +
    "Then run **/verify** again. If it still does not work, ask an admin to manually verify you."
  );
}

async function commandManualVerify(interaction, ownerDiscordId, env) {
  const targetDiscordId = getUserOption(interaction, "user");
  if (!targetDiscordId) {
    return "Choose a Discord member to verify.";
  }

  const roleResult = await changeDiscordRole(targetDiscordId, true, env);
  if (!roleResult.success) {
    return `I could not add the Training Dojo role to <@${targetDiscordId}>. Check that they are in the server and that the bot role is above the Dojo role.`;
  }

  if (env.MEMBER_LINKS) {
    await env.MEMBER_LINKS.put(
      `manual:${targetDiscordId}`,
      JSON.stringify({
        granted_by: ownerDiscordId,
        granted_at: new Date().toISOString(),
      }),
      { expirationTtl: MANUAL_VERIFY_TTL_SECONDS },
    );
  }

  if (env.RR_TRACKER) {
    try {
      await env.RR_TRACKER.setMemberActive(targetDiscordId, true);
    } catch (error) {
      console.warn("Could not mark manually verified member active in RR tracker:", error);
    }
  }

  return `Manually verified <@${targetDiscordId}>. Their Dojo role is active for up to **7 days** while they fix their Whop connection.`;
}

async function commandManualUnverify(interaction, ownerDiscordId, env) {
  const targetDiscordId = getUserOption(interaction, "user");
  if (!targetDiscordId) {
    return "Choose a Discord member to unverify.";
  }

  if (env.MEMBER_LINKS) {
    await env.MEMBER_LINKS.delete(`manual:${targetDiscordId}`);
  }

  const roleResult = await changeDiscordRole(targetDiscordId, false, env);

  if (env.RR_TRACKER) {
    try {
      await env.RR_TRACKER.setMemberActive(targetDiscordId, false);
    } catch (error) {
      console.warn("Could not mark manually unverified member inactive in RR tracker:", error);
    }
  }

  if (!roleResult.success && roleResult.status !== 404) {
    return `The manual override was removed, but Discord returned an error while removing the role from <@${targetDiscordId}>.`;
  }

  return `Manual verification removed for <@${targetDiscordId}>. If they still have an active Whop membership, automatic verification can add the role again.`;
}
...
function formatRrStats(result) {
  if (!result?.ok) {
    if (result?.code === "NOT_LINKED") {
      return "No Riot account is linked yet. Use **/linkriot** first.";
    }
...
async function ensureGuildCommands(env) {
  if (!env.DISCORD_APP_ID || !env.DISCORD_GUILD_ID || !env.DISCORD_BOT_TOKEN || !env.MEMBER_LINKS) {
...
function findDiscordAccountId(accounts) {
  if (!Array.isArray(accounts)) return null;

  for (const account of accounts) {
    const service = String(account?.platform || account?.service || account?.provider || "").toLowerCase();
    const accountId = account?.external_id || account?.account_id || null;
    if (service === "discord" && accountId) {
...
async function getDiscordIdFromWhop(userId, env) {
  try {
    ...
    const legacyAccounts =
      user?.social_accounts || user?.data?.social_accounts || user?.accounts || [];
    const legacyId = findDiscordAccountId(legacyAccounts);
    if (legacyId) return legacyId;
  } catch (error) {
    console.warn(`Legacy Whop social account lookup failed for ${userId}:`, error);
  }
  ...
  for (const url of candidateUrls) {
    try {
      ...
      const accounts = Array.isArray(body) ? body : body?.data || body?.social_accounts || [];
      const discordId = findDiscordAccountId(accounts);
      if (discordId) return discordId;
    } catch (error) {
      console.warn(`Whop v5 social account lookup failed for ${userId}:`, error);
    }
  }
...
async function hasManualOverride(discordId, env) {
  if (!env.MEMBER_LINKS || !discordId) return false;
  try {
    return Boolean(await env.MEMBER_LINKS.get(`manual:${discordId}`));
  } catch {
    return false;
  }
}

async function clearManualOverride(discordId, env) {
  if (!env.MEMBER_LINKS || !discordId) return;
  try {
    await env.MEMBER_LINKS.delete(`manual:${discordId}`);
  } catch (error) {
    console.warn("Could not clear manual verification override:", error);
  }
}
...
async function syncUserRole(userId, shouldHaveAccess, env) {
  ...
  if (!shouldHaveAccess) {
    if (discordId === String(env.DISCORD_OWNER_USER_ID || "")) {
      return { success: true, skipped: "owner" };
    }

    if (await hasManualOverride(discordId, env)) {
      return { success: true, skipped: "manual_override" };
    }
  }
  ...
  if (result.success) {
    ...
    if (shouldHaveAccess) {
      await clearManualOverride(discordId, env);
    }
  ...
  if (cached?.whop_user_id) {
    ...
    if (membership) {
      ...
      if (roleResult.success) {
        ...
        await clearManualOverride(discordUserId, env);
  ...
  if (env.MEMBER_LINKS) {
    ...
    do {
      ...
      for (const key of listed.keys || []) {
        ...
        if (discordId === String(env.DISCORD_OWNER_USER_ID || "")) continue;
        if (await hasManualOverride(discordId, env)) continue;
