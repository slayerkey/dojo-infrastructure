import legacy, { DiscordGateway as DiscordGatewayV20 } from "./index-v20.js";

const DISCORD_API = "https://discord.com/api/v10";
const EPHEMERAL = 64;
const VALORANT_RED = 0xff4655;
const PAGE_SIZE = 5;
const COMPONENT_PREFIX = "rrlb:v2";
      ...
      ctx.waitUntil(
        loadLeaderboardSession(env, component.ownerId, component.sessionId)
          .then(({ snapshot, cacheSource }) => {
            ...
            const payload = renderLeaderboardSnapshot(
              snapshot,
              component.ownerId,
              component.sessionId,
              requestedPage,
            ...
            return editOriginalInteraction(interaction, env, payload)
    ...
    if (url.pathname === "/health") {
      ...
      try {
        ...
        body.discord.rrleaderboard = {
          mode: "five-player-memory-snapshot-pagination-v21",
          page_size: PAGE_SIZE,
          controls: ["previous", "me", "next"],
          snapshot_ttl_minutes: SNAPSHOT_TTL_MS / 60000,
          snapshot_storage: "durable-object-memory",
          kv_state: false,
          d1_writes: false,
          page_click_d1_reads_on_cache_hit: false,
        };
...
export class DiscordGateway extends DiscordGatewayV20 {
  constructor(ctx, env) {
    super(ctx, env);
    this.rrLeaderboardSnapshots = new Map();
  }

  async getRrLeaderboardSnapshot(sessionId, ownerId) {
    this.pruneRrLeaderboardSnapshots();
    const id = String(sessionId || "");
    const item = this.rrLeaderboardSnapshots.get(id);
    if (!item) return null;
    if (String(item.ownerId || "") !== String(ownerId || "")) return null;
    if (Number(item.expiresAt || 0) <= Date.now()) {
      this.rrLeaderboardSnapshots.delete(id);
      return null;
    }
    return item.snapshot || null;
  }

  async putRrLeaderboardSnapshot(sessionId, ownerId, snapshot) {
    this.pruneRrLeaderboardSnapshots();
    const id = String(sessionId || "");
    if (!id || !ownerId || !snapshot?.ok) return { ok: false };

    if (!this.rrLeaderboardSnapshots.has(id) && this.rrLeaderboardSnapshots.size >= MAX_SNAPSHOTS) {
      const oldest = this.rrLeaderboardSnapshots.keys().next().value;
      if (oldest) this.rrLeaderboardSnapshots.delete(oldest);
    }

    this.rrLeaderboardSnapshots.set(id, {
  ...
  pruneRrLeaderboardSnapshots() {
    const now = Date.now();
    for (const [key, item] of this.rrLeaderboardSnapshots.entries()) {
      if (Number(item?.expiresAt || 0) <= now) {
        this.rrLeaderboardSnapshots.delete(key);
      }
    }
  }
}
...
async function createLeaderboardSession(env, ownerId, sessionId) {
  ...
  if (!snapshot?.ok) {
    throw new Error(snapshot?.message || "Leaderboard snapshot failed");
  }
...
async function loadLeaderboardSession(env, ownerId, sessionId) {
  ...
  if (stub) {
    const cached = await stub
      .getRrLeaderboardSnapshot(sessionId, ownerId)
      .catch(() => null);
    if (cached?.ok) {
      return { snapshot: cached, cacheSource: "durable_object_memory" };
    }
  }

  const snapshot = await env.RR_TRACKER.getLeaderboardSnapshot(ownerId);
  if (!snapshot?.ok) {
    throw new Error(snapshot?.message || "Leaderboard snapshot refresh failed");
  }
  await cacheLeaderboardSnapshot(env, sessionId, ownerId, snapshot);
...
function formatLeaderboardEntry(entry) {
  ...
  // Keep the position/name bold separately from the already-bold RR delta.
  // Nesting **...${delta}...** caused Discord to leak literal ** and wrap the RR
  // onto its own line for positions 4+.
