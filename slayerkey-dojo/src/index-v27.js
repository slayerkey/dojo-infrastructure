import legacy, { DiscordGateway as DiscordGatewayV26 } from "./index-v26.js";

const DISCORD_API = "https://discord.com/api/v10";
const EPHEMERAL = 64;
const encoder = new TextEncoder();
const PHOENIX_OFFSET_MS = 7 * 60 * 60 * 1000;
const DEFAULT_ACTIVITY_CHANNEL_ID = "1545559455540715602";
const DEFAULT_ACTIVITY_WEEKDAY = 0; // Sunday
const DEFAULT_ACTIVITY_HOUR = 6;
const DEFAULT_ACTIVITY_MINUTE = 0;

const EXTENDED_ROLE_DEFINITIONS = [
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/discord/interactions" && request.method === "POST") {
      ...
      ctx.waitUntil(
        runExtendedRoleSync(env)
          .then((summary) => editOriginalInteraction(interaction, env, {
            content:
              `Role sync complete. **${summary.roles_created}** role(s) created, ` +
              `**${summary.members_synced}** active Dojo member(s) synced, ` +
              `**${summary.annual_members}** annual member(s) detected, ` +
              `**${summary.unresolved_members}** active Whop member(s) could not be matched to Discord.`,
          }))
          .catch(async (error) => {
            console.error("memberroles v27 failed:", error);
    ...
    if (url.pathname === "/health") {
      ...
      try {
        ...
        body.discord.membership_roles = {
          ...
          tenure_roles: EXTENDED_ROLE_DEFINITIONS.filter((item) => item.key !== "annual").map((item) => item.name),
          first_month_role_is_immediate: true,
          single_tenure_role: true,
          preserved_tenure: true,
          daily_sync: true,
        };
        body.discord.activity_check = {
          ...(body.discord.activity_check || {}),
          default_channel_id: DEFAULT_ACTIVITY_CHANNEL_ID,
          default_schedule: "Sunday 6:00 AM Arizona time",
          first_automatic_report_after_full_week: true,
        };
  ...
  async scheduled(controller, env, ctx) {
    const tasks = [runV27Maintenance(env)];
...
export class DiscordGateway extends DiscordGatewayV26 {
  async claimExtendedRoleSyncDay(dayKey) {
    const key = "membership_v27:last_day";
...
async function runV27Maintenance(env) {
  const stub = env.DISCORD_GATEWAY?.getByName("dojo-main");
  if (!stub) return;
  const now = new Date();
  const dayKey = now.toISOString().slice(0, 10);

  const claimed = await stub.claimExtendedRoleSyncDay(dayKey).catch(() => false);
  if (claimed) {
    await ensureAdminCommands(env).catch((error) => console.error("v27 command registration failed:", error));
    await runExtendedRoleSync(env).catch((error) => console.error("v27 role sync failed:", error));
  }

  await ensureDefaultActivitySchedule(env, stub, now).catch((error) => {
    console.error("Default activity schedule setup failed:", error);
  });
}

async function ensureDefaultActivitySchedule(env, stub, now) {
  const existing = await stub.getActivityConfig().catch(() => null);
  if (existing?.enabled && existing.channel_id) return;
  const configuredAt = now.toISOString();
  await stub.setActivityConfig({
    enabled: true,
    channel_id: DEFAULT_ACTIVITY_CHANNEL_ID,
    weekday: DEFAULT_ACTIVITY_WEEKDAY,
    hour: DEFAULT_ACTIVITY_HOUR,
    minute: DEFAULT_ACTIVITY_MINUTE,
    configured_at: configuredAt,
    source: "v27_default",
  });
}

async function runExtendedRoleSync(env) {
  requireRoleSyncEnv(env);
  const { roles, created } = await ensureExtendedRoles(env);
  const memberships = await fetchCurrentDojoMemberships(env);
  const activeMemberships = memberships.filter(isActiveMembership);
  const grouped = groupMembershipsByUser(activeMemberships);
  const activeWhopUsers = new Set(grouped.keys());
  const stub = env.DISCORD_GATEWAY.getByName("dojo-main");
  ...
  for (const [whopUserId, userMemberships] of grouped.entries()) {
    ...
    const annualFlags = await Promise.all(
      userMemberships.map((membership) => isAnnualMembership(membership, env, planCache)),
    );
    ...
    const joinedAt = earliestMembershipDate(userMemberships);
    ...
    await applyExtendedRoles(discordUserId, next, roles, env);
  ...
  for (const record of Array.isArray(known) ? known : []) {
    if (!record?.active || activeWhopUsers.has(String(record.whop_user_id || ""))) continue;
    const discordUserId = String(record.discord_user_id || "");
    if (!discordUserId) continue;
    const next = { ...record, active: false, is_annual: false, updated_at: new Date().toISOString() };
    await stub.putTenureRecord(discordUserId, next);
    await applyExtendedRoles(discordUserId, next, roles, env);
...
async function applyExtendedRoles(discordUserId, record, roles, env) {
  ...
  if (record?.active) {
    ...
    const tenureKey = extendedTenureRoleKey(record.first_eligible_at);
  ...
  for (const roleId of Object.values(roles).map(String)) {
    ...
    await changeRole(discordUserId, roleId, shouldHave, env, "Automatic Dojo tenure/annual sync");
  }
  return { ok: true };
}

function extendedTenureRoleKey(firstEligibleAt, now = new Date()) {
  ...
  // Month 1 means the member is currently in their first month, not that they
  // have already completed one full month. This is the intended visible progression.
...
async function isAnnualMembership(membership, env, planCache) {
  ...
  const candidates = [
    membership?.renewal_period_start && membership?.renewal_period_end
      ? [membership.renewal_period_start, membership.renewal_period_end]
      : null,
    membership?.created_at && membership?.expires_at
      ? [membership.created_at, membership.expires_at]
      : null,
  ].filter(Boolean);
  ...
  for (const [startValue, endValue] of candidates) {
    ...
    if (Number.isFinite(start) && Number.isFinite(end) && (end - start) / 86400000 >= 300) return true;
  }
  ...
  if (!planCache.has(planId)) {
    planCache.set(planId, fetchWhopPlan(planId, env).catch(() => null));
  }
  return looksAnnual(await planCache.get(planId));
}
...
async function fetchWhopPlan(planId, env) {
  const response = await fetch(`https://api.whop.com/api/v1/plans/${encodeURIComponent(planId)}`, {
...
async function fetchPaged(path, env, configure) {
  ...
  do {
    ...
    const response = await fetch(`https://api.whop.com/api/v1${path}?${params.toString()}`, {
      headers: { Authorization: `Bearer ${env.WHOP_API_KEY}` },
    });
...
function requireRoleSyncEnv(env) {
